...
const markdownElements = document.querySelectorAll("md"); // Gets all markdown elements on the page (<md>)

// Checks if there ARE any <md> elements on the page
if(markdownElements.length > 0) {
    markdownElements.forEach((mdElement) => {
        const markdownText = mdElement.innerHTML; // selects the innerHTML of the <md> element
        const convertedHtml = parseMarkdown(markdownText); // converts it to md
        mdElement.innerHTML = convertedHtml;
    });
}